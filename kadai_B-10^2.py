...
def update():
    global ballx, bally, vx, vy,padx,a,end
    padx=pyxel.mouse_x
    for i in range(0,len(ballx)) :
     ...
     if ballx[i]<=0 :
        vx[i]=0.866
     if bally[i]>=194.8 and padx-20<=ballx[i]<=padx+20 :
        a+=1
        pyxel.play(0, [0,0])
     if bally[i]>=194.8 and (padx-20>ballx[i] or padx+20<ballx[i]) :
        pyxel.play(1, [1,1])
        end +=1
def draw():
    global ballx, bally, vx, vy, padx,a,end
    ...
    pyxel.text(20,20,"miss:"+str(end),0)
pyxel.run(update, draw)
# end変数を追加し、これが１０を超えるとゲームを終了するようにしました。