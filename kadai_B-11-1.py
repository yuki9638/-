...
s=1
def update():
    global padx,score,angle,error,s,speed
    for i in range (len(ball)) :
     ...
     if ball[i].ballx<=0 :
        ball[i].vx=-ball[i].vx
     if ball[i].bally>=190 and padx-20<=ball[i].ballx<=padx+20 :
        score+=1
        s+=1
        pyxel.play(0, [0,0])
     if ball[i].bally>=194.3 and (padx-20>ball[i].ballx or padx+20<ball[i].ballx) :
        pyxel.play(1, [1,1])
        error +=1
     if s % 11 == 0  :
...
# ボールをクラスにしてリストのインスタンスとして呼び出しました